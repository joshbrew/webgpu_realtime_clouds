...
`;
  ...
  var CloudComputeBuilder = class {
    ...
    setTuning(t = {}) {
      ...
      putF(224, s.minOutputAlpha ?? 0.1);
      putF(228, s.outputAlphaFeather ?? 0);
    ...
    _writeRenderUniforms(opts = {}) {
      ...
      const silverExponent = opts.silverExponent ?? 1;
      ...
      const alphaFloor = Math.max(0, Math.min(1, opts.alphaFloor ?? 0.2));
...
`;
  ...
  function renderUniformSignature(preview, aspect, layerIndex, cloudParams = {}) {
    return JSON.stringify({
      ...
      alphaFloor: signatureScalar(preview?.alphaFloor ?? 0.2),
      fogDensity: signatureScalar(preview?.fogDensity ?? 0.34),
      fogHorizon: signatureScalar(preview?.fogHorizon ?? 0.3),
      fogSun: signatureScalar(preview?.fogSun ?? 1.5),
      silverIntensity: signatureScalar(cloudParams?.silverIntensity ?? 1.5),
      silverExponent: signatureScalar(cloudParams?.silverExponent ?? 1)
    });
  }
  ...
  function autoThickBoxTuning(box) {
    ...
    return {
      ...
      thickDetailSkip: +(0.09 + thickF * 0.26).toFixed(4),
  ...
  async function runFrame({
    ...
    if (renderSig !== lastRenderUniformSignature) {
      cb._writeRenderUniforms({
        ...
        silverExponent: cloudParams?.silverExponent ?? 1,
        godRaysEnabled: !!preview?.godRaysEnabled,
        ...
        alphaFloor: preview?.alphaFloor ?? 0.2,
