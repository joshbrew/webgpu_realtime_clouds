...
fn voronoiCircleGradient(pos: vec3<f32>, params: NoiseParams) -> f32 {
    ...
    // search the 3\xD73\xD73 neighborhood
    for (var dz: i32 = -1; dz <= 1; dz = dz + 1) {
        for (var dy: i32 = -1; dy <= 1; dy = dy + 1) {
            for (var dx: i32 = -1; dx <= 1; dx = dx + 1) {
                let xi = fx + dx;
...
`;
  ...
  function normalizeReproj(r) {
    ...
    const out = {
      ...
      temporalBlend: typeof r.temporalBlend === "number" ? r.temporalBlend : r.enabled ? 0.82 : 0,
  ...
  function cloneReprojForReset(r) {
    ...
    return Object.assign({}, r, {
      ...
      temporalBlend: 0,
      frameIndex: 0,
      scale: typeof r.scale === "number" ? r.scale : 1 / f32Like(ss * ss),
