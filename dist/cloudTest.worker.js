...
`;
  ...
  var CloudComputeBuilder = class {
    ...
    _writeRenderUniforms(opts = {}) {
      ...
      const styleSaturation = opts.styleSaturation ?? 1.1;
      const styleRimStrength = opts.styleRimStrength ?? 1;
      const styleSunBleed = opts.styleSunBleed ?? 0.85;
      const styleMidLift = opts.styleMidLift ?? 1.1;
...
`;
  ...
  async function runFrame({
    ...
    const enc = device.createCommandEncoder();
    const tC0 = performance.now();
    const encodedDispatch = typeof cb.encodeDispatchPasses === "function" ? cb.encodeDispatchPasses(enc, { coarseFactor: cf, skipUpsampleForPreview: false }) : null;
    if (!encodedDispatch) {
