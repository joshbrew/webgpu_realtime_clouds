...
export class CloudComputeBuilder {
  constructor(device, queue) {
    ...
    this.width = 0; this.height = 0; this.layers = 0;

    // coarse
    this._coarseTexture = null; this._coarseView = null; this._coarseW = 0; this._coarseH = 0; this._coarseLayers = 0;

    // workgroups
    this._wgX = 1; this._wgY = 1;

    // GPU objects filled in _init*
    this.module = null; this.pipeline = null;
    this.bgl0 = null; this.bgl1 = null;

    // samplers
    this._samp2D = null; this._sampShape = null; this._sampDetail = null; this._sampBN = null;

    // buffers (UBOs, staging ABs)
    // sizes chosen to match prior layout, keep for compatibility
    this._abOptions = new ArrayBuffer(32); this._dvOptions = new DataView(this._abOptions);

    // NOTE: extended CloudParams to 96 bytes to include shapeScale & detailScale
    this._abParams = new ArrayBuffer(96); this._dvParams = new DataView(this._abParams); // CloudParams (96 bytes)

    this._abOffsets = new ArrayBuffer(48); this._dvOffsets = new DataView(this._abOffsets);
    this._abFrame = new ArrayBuffer(64); this._dvFrame = new DataView(this._abFrame);
    this._abLight = new ArrayBuffer(32); this._dvLight = new DataView(this._abLight);
    this._abView = new ArrayBuffer(128); this._dvView = new DataView(this._abView);
    this._abBox = new ArrayBuffer(32); this._dvBox = new DataView(this._abBox);
    this._abSampling = new ArrayBuffer(16); this._dvSampling = new DataView(this._abSampling);
    this._abReproj = new ArrayBuffer(48); this._dvReproj = new DataView(this._abReproj);
    this._abPerf = new ArrayBuffer(16); this._dvPerf = new DataView(this._abPerf);

    // TUNE staging - roomy (256) to match prior
    this._abTuning = new ArrayBuffer(256); this._dvTuning = new DataView(this._abTuning);
    ...
    this.optionsBuffer = null; this.paramsBuffer = null; this.offsetsBuffer = null;
    this.dummyBuffer = null; this.posBuffer = null; this.frameBuffer = null; this.lightBuffer = null; this.viewBuffer = null; this.boxBuffer = null;
    this.samplingBuffer = null; this.reprojBuffer = null; this.perfBuffer = null; this.tuningBuffer = null; this.renderParams = null;

    // bookkeeping & caches
    this._lastSums = new Map();
    this._resId = new WeakMap(); this._nextResId = 1;
    this._bg0Cache = new Map(); this._bg0Keys = []; this._bg1Cache = new Map(); this._bg1Keys = [];
    this._bg0Dirty = true; this._bg1Dirty = true; this._currentBg0 = null; this._currentBg1 = null;
    this._render = null; this._ctxCache = new WeakMap(); this._canvasStates = new WeakMap(); this._renderBgCache = new WeakMap(); this._renderBundleCache = new WeakMap();
    this._upsampleBgCache = new Map();

    // tiny dummies
    this._ownsBlue = false;
    this._dummy2DMotion = null; this._dummy2DMotionView = null;
    this._dummy2DDepth = null; this._dummy2DDepthView = null;
    this._dummyHistoryPrev = null; this._dummyHistoryPrevView = null;
    this._dummyHistoryOut = null; this._dummyHistoryOutView = null;

    // reproj tracking
    this._lastHadWork = false;
    this._reprojFullW = 0; this._reprojFullH = 0;
    ...
    this.setReprojSettings({ enabled: 0, subsample: 1, sampleOffset: 0, motionIsNormalized: 0, temporalBlend: 0.0, depthTest: 0, depthTolerance: 0.0, frameIndex: 0, fullWidth: 0, fullHeight: 0 });
    this.setPerfParams();
    this.setSunByAngles();
    this.setBox();
  ...
  _initCompute() {
    ...
    this.bgl0 = d.createBindGroupLayout({
      entries: [
        ...
        { binding: 2, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } }, // dummy storage
        { binding: 3, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'uniform' } }, // offsets
        { binding: 4, visibility: GPUShaderStage.COMPUTE, storageTexture: { access: 'write-only', format: this.outFormat, viewDimension: '2d-array' } }, // out
        { binding: 5, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'read-only-storage' } }, // pos
        { binding: 6, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'uniform' } }, // frame
        { binding: 7, visibility: GPUShaderStage.COMPUTE, storageTexture: { access: 'write-only', format: this.outFormat, viewDimension: '2d-array' } }, // historyOut or dummy
        { binding: 8, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'uniform' } }, // reproj
        { binding: 9, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'uniform' } }, // perf
        { binding: 10, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'uniform' } } // tuning
      ]
    });
    ...
    this.bgl1 = d.createBindGroupLayout({
      entries: [
        { binding: 0, visibility: GPUShaderStage.COMPUTE, texture: { sampleType: 'float', viewDimension: '2d-array' } }, // weather
        { binding: 1, visibility: GPUShaderStage.COMPUTE, sampler: { type: 'filtering' } },
        { binding: 2, visibility: GPUShaderStage.COMPUTE, texture: { sampleType: 'float', viewDimension: '3d' } }, // shape3D
        { binding: 3, visibility: GPUShaderStage.COMPUTE, sampler: { type: 'filtering' } },
        { binding: 4, visibility: GPUShaderStage.COMPUTE, texture: { sampleType: 'float', viewDimension: '2d-array' } }, // blue
        { binding: 5, visibility: GPUShaderStage.COMPUTE, sampler: { type: 'filtering' } },
        { binding: 6, visibility: GPUShaderStage.COMPUTE, texture: { sampleType: 'float', viewDimension: '3d' } }, // detail3D
        { binding: 7, visibility: GPUShaderStage.COMPUTE, sampler: { type: 'filtering' } },
        { binding: 8, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'uniform' } }, // light
        { binding: 9, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'uniform' } }, // view
        { binding: 10, visibility: GPUShaderStage.COMPUTE, buffer: { type: 'uniform' } }, // box
        { binding: 11, visibility: GPUShaderStage.COMPUTE, texture: { sampleType: 'float', viewDimension: '2d-array' } }, // historyPrev
        { binding: 12, visibility: GPUShaderStage.COMPUTE, sampler: { type: 'filtering' } },
        { binding: 13, visibility: GPUShaderStage.COMPUTE, texture: { sampleType: 'float', viewDimension: '2d' } }, // motion
        { binding: 14, visibility: GPUShaderStage.COMPUTE, sampler: { type: 'filtering' } },
        { binding: 15, visibility: GPUShaderStage.COMPUTE, texture: { sampleType: 'float', viewDimension: '2d' } }, // depthPrev
  ...
  // -------------------- UBO setters --------------------
  setOptions({ useCustomPos = false, outputChannel = 0, writeRGB = true, debugForceFog = 0, temporalSeed = 0, windDisp = 0.0, windAngleRad = 0.0 } = {}) {
  ...
  setTemporalSeed(seed = 0) { this._dvOptions.setFloat32(20, seed, true); this._writeIfChanged('options', this.optionsBuffer, this._abOptions); this._bg0Dirty = true; }

  setParams(p = {}) {
    // defaults updated to include the new CloudParams tunables
    const defaults = {
      globalCoverage: 0.6, globalDensity: 1.0,
      cloudAnvilAmount: 0.0, cloudBeer: 6.0,
      attenuationClamp: 0.2, inScatterG: 0.2,
      silverIntensity: 2.5, silverExponent: 2.0,
      outScatterG: 0.1, inVsOut: 0.5,
      outScatterAmbientAmt: 0.9, ambientMinimum: 0.2,
  ...
  setWeatherLayer(layer = 0) { this.setSamplingOpts({ useManualWrap: this._dvSampling.getUint32(0, true), weatherLayer: layer }); }
  setManualWrap(on = true) { this.setSamplingOpts({ useManualWrap: on ? 1 : 0, weatherLayer: this.getWeatherLayer() }); }
  getWeatherLayer() { return this._dvSampling.getUint32(4, true) || 0; }

  setReprojSettings({ enabled = 0, subsample = 1, sampleOffset = 0, motionIsNormalized = 0, temporalBlend = 0.0, depthTest = 0, depthTolerance = 0.0, frameIndex = 0, fullWidth = 0, fullHeight = 0 } = {}) {
    ...
    if (fullWidth) { dv.setUint32(32, fullWidth >>> 0, true); this._reprojFullW = fullWidth; }
    if (fullHeight) { dv.setUint32(36, fullHeight >>> 0, true); this._reprojFullH = fullHeight; }
    this._writeIfChanged('reproj', this.reprojBuffer, this._abReproj);
    this._bg0Dirty = true;
  }
  setReprojFullResSize(fullWidth, fullHeight) {
    this._reprojFullW = fullWidth | 0; this._reprojFullH = fullHeight | 0;
  ...
  setLight({ sunDir = [-0.4, 0.8, 0.45], camPos = [0, 0, 2] } = {}) {
    ...
    dv.setFloat32(0, sunDir[0], true); dv.setFloat32(4, sunDir[1], true); dv.setFloat32(8, sunDir[2], true); dv.setFloat32(12, 0.0, true);
    dv.setFloat32(16, camPos[0], true); dv.setFloat32(20, camPos[1], true); dv.setFloat32(24, camPos[2], true); dv.setFloat32(28, 0.0, true);
    this._writeIfChanged('light', this.lightBuffer, this._abLight);
    this._bg1Dirty = true;
  }
  setSunByAngles({ azimuthDeg = 45, elevationDeg = 35, camPos = [0, 0, 2] } = {}) {
    const az = (azimuthDeg * Math.PI) / 180, el = (elevationDeg * Math.PI) / 180; const sd = [Math.cos(el) * Math.sin(az), Math.sin(el), Math.cos(el) * Math.cos(az)];
    this.setLight({ sunDir: sd, camPos });
  }

  setBox({ center = [0, 0, 0], half = [1, 0.6, 1], uvScale = 1.5 } = {}) {
    const dv = this._dvBox;
    dv.setFloat32(0, center[0], true); dv.setFloat32(4, center[1], true); dv.setFloat32(8, center[2], true); dv.setFloat32(12, 0.0, true);
    dv.setFloat32(16, half[0], true); dv.setFloat32(20, half[1], true); dv.setFloat32(24, half[2], true); dv.setFloat32(28, uvScale, true);
    this._writeIfChanged('box', this.boxBuffer, this._abBox);
    this._bg1Dirty = true;
  }

  setFrame({ fullWidth = 0, fullHeight = 0, tileWidth = 0, tileHeight = 0, originX = 0, originY = 0, originZ = 0, fullDepth = 1, tileDepth = 1, layerIndex = 0, layers = 1, originXf = 0.0, originYf = 0.0 } = {}) {
    const dv = this._dvFrame;

    dv.setUint32(0, fullWidth >>> 0, true); dv.setUint32(4, fullHeight >>> 0, true);
    dv.setUint32(8, tileWidth >>> 0, true); dv.setUint32(12, tileHeight >>> 0, true);
    dv.setInt32(16, originX | 0, true); dv.setInt32(20, originY | 0, true); dv.setInt32(24, originZ | 0, true);
    dv.setUint32(28, fullDepth >>> 0, true); dv.setUint32(32, tileDepth >>> 0, true);
    dv.setInt32(36, layerIndex | 0, true); dv.setUint32(40, layers >>> 0, true); dv.setUint32(44, 0, true);
    dv.setFloat32(48, originXf ?? 0.0, true); dv.setFloat32(52, originYf ?? 0.0, true);
    dv.setFloat32(56, 0.0, true); dv.setFloat32(60, 0.0, true);
    this._writeIfChanged('frame', this.frameBuffer, this._abFrame);

    const w = tileWidth || fullWidth; const h = tileHeight || fullHeight;
    if (w && h) { this._wgX = Math.max(1, Math.ceil(w / 8)); this._wgY = Math.max(1, Math.ceil(h / 8)); }
  }
  setLayerIndex(i) { this._dvFrame.setInt32(36, i | 0, true); this._writeIfChanged('frame', this.frameBuffer, this._abFrame); }

  setViewFromCamera({ camPos = [0, 0, 3], right = [1, 0, 0], up = [0, 1, 0], fwd = [0, 0, 1], fovYDeg = 60, aspect = 1, planetRadius = 0.0, cloudBottom = -1.0, cloudTop = 1.0, worldToUV = 1.0, stepBase = 0.02, stepInc = 0.04, volumeLayers = 1 } = {}) {
    const dv = this._dvView;
    const norm = v => { const L = Math.hypot(v[0], v[1], v[2]) || 1; return [v[0] / L, v[1] / L, v[2] / L]; };
    const f = norm(fwd); const u0 = norm(up);
    const r = norm([u0[1] * f[2] - u0[2] * f[1], u0[2] * f[0] - u0[0] * f[2], u0[0] * f[1] - u0[1] * f[0]]);
    ...
    const floats = [
      ...
      u[0], u[1], u[2], 0,
      f[0], f[1], f[2], 0,
      (fovYDeg * Math.PI / 180), aspect, stepBase, stepInc,
      planetRadius, cloudBottom, cloudTop, volumeLayers,
      worldToUV, 0, 0, 0, 0, 0, 0, 0
    ];
    for (let i = 0; i < floats.length; i++) dv.setFloat32(i * 4, floats[i], true);
    this._writeIfChanged('view', this.viewBuffer, this._abView);
    this._bg1Dirty = true;
  }
  setSunByDirection({ dir = [0, 1, 0], camPos = [0, 0, 3] } = {}) { this.setLight({ sunDir: dir, camPos }); }

  // -------------------- TUNE setter --------------------
  setTuning(t = {}) {
    // defaults (kept consistent with previous) + new styleBlend default
    const defaults = {
      maxSteps: 256, minStep: 0.003, maxStep: 0.10,
      sunSteps: 4, sunStride: 4, sunMinTr: 0.005,
      phaseJitter: 1.0, stepJitter: 0.08,
      baseJitterFrac: 0.15, topJitterFrac: 0.10,
      lodBiasWeather: 1.5, aabbFaceOffset: 0.0015,
      weatherRejectGate: 0.99, weatherRejectMip: 1.0, emptySkipMult: 3.0,
      nearFluffDist: 60.0, nearStepScale: 0.30, nearLodBias: -1.5, nearDensityMult: 2.5, nearDensityRange: 45.0,
      lodBlendThreshold: 0.05,
      sunDensityGate: 0.015, fflyRelClamp: 2.5, fflyAbsFloor: 1.5, taaRelMin: 0.22, taaRelMax: 1.10, taaAbsEps: 0.02,
      farStart: 800.0, farFull: 2500.0, farLodPush: 0.8, farDetailAtten: 0.50, farStepMult: 1.6, bnFarScale: 0.35, farTaaHistoryBoost: 1.35,
      raySmoothDens: 0.50, raySmoothSun: 0.50,
    ...
    let o = 0;
    dv.setInt32(o, c.maxSteps | 0, true); o += 4;
    dv.setInt32(o, 0, true); o += 4; // pad
    dv.setFloat32(o, c.minStep, true); o += 4;
    dv.setFloat32(o, c.maxStep, true); o += 4;

    dv.setInt32(o, c.sunSteps | 0, true); o += 4;
    dv.setInt32(o, c.sunStride | 0, true); o += 4;
    dv.setFloat32(o, c.sunMinTr, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;

    dv.setFloat32(o, c.phaseJitter, true); o += 4;
    dv.setFloat32(o, c.stepJitter, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;

    dv.setFloat32(o, c.baseJitterFrac, true); o += 4;
    dv.setFloat32(o, c.topJitterFrac, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;

    dv.setFloat32(o, c.lodBiasWeather, true); o += 4;
    dv.setFloat32(o, c.aabbFaceOffset, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;

    dv.setFloat32(o, c.weatherRejectGate, true); o += 4;
    dv.setFloat32(o, c.weatherRejectMip, true); o += 4;
    dv.setFloat32(o, c.emptySkipMult, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;

    dv.setFloat32(o, c.nearFluffDist, true); o += 4;
    dv.setFloat32(o, c.nearStepScale, true); o += 4;
    dv.setFloat32(o, c.nearLodBias, true); o += 4;
    dv.setFloat32(o, c.nearDensityMult, true); o += 4;
    dv.setFloat32(o, c.nearDensityRange, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;

    dv.setFloat32(o, c.lodBlendThreshold, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;

    dv.setFloat32(o, c.sunDensityGate, true); o += 4;
    dv.setFloat32(o, c.fflyRelClamp, true); o += 4;
    dv.setFloat32(o, c.fflyAbsFloor, true); o += 4;
    dv.setFloat32(o, c.taaRelMin, true); o += 4;
    dv.setFloat32(o, c.taaRelMax, true); o += 4;
    dv.setFloat32(o, c.taaAbsEps, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;

    dv.setFloat32(o, c.farStart, true); o += 4;
    dv.setFloat32(o, c.farFull, true); o += 4;
    dv.setFloat32(o, c.farLodPush, true); o += 4;
    dv.setFloat32(o, c.farDetailAtten, true); o += 4;
    dv.setFloat32(o, c.farStepMult, true); o += 4;
    dv.setFloat32(o, c.bnFarScale, true); o += 4;
    dv.setFloat32(o, c.farTaaHistoryBoost, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;
    dv.setFloat32(o, 0.0, true); o += 4;

    dv.setFloat32(o, c.raySmoothDens, true); o += 4;
    dv.setFloat32(o, c.raySmoothSun, true); o += 4;

    // NEW: styleBlend float (0..1) to morph between old(flat) and new(bulgy) styles
    dv.setFloat32(o, c.styleBlend, true); o += 4;
  ...
  setInputMaps({ weatherView, shape3DView, detail3DView, blueTex, blueView, motionView, depthPrevView, historyPrevView } = {}) {
    let changed = false;
    if (typeof weatherView !== 'undefined' && weatherView !== this.weatherView) { this.weatherView = weatherView; changed = true; }
    if (typeof shape3DView !== 'undefined' && shape3DView !== this.shape3DView) { this.shape3DView = shape3DView; changed = true; }
    if (typeof detail3DView !== 'undefined' && detail3DView !== this.detail3DView) { this.detail3DView = detail3DView; changed = true; }
    if (typeof blueView !== 'undefined' && blueView !== this.blueView) { this.blueView = blueView; changed = true; }
    if (typeof blueTex !== 'undefined' && blueTex !== this.blueTex) { this.blueTex = blueTex; this._ownsBlue = false; this.blueView = null; changed = true; }
    if (typeof motionView !== 'undefined' && motionView !== this.motionView) { this.motionView = motionView; changed = true; }
    if (typeof depthPrevView !== 'undefined' && depthPrevView !== this.depthPrevView) { this.depthPrevView = depthPrevView; changed = true; }
    if (typeof historyPrevView !== 'undefined' && historyPrevView !== this.historyPrevView) { this.historyPrevView = historyPrevView; changed = true; }
    if (changed) this._bg1Dirty = true;
  }

  setHistoryPrevView(view) { if (view !== this.historyPrevView) { this.historyPrevView = view; this._bg1Dirty = true; } }
  setHistoryOutView(view) { this.historyOutView = view; this._bg0Dirty = true; }
  ...
  createOutputTexture(width, height, layers = 1, format = 'rgba16float') {
    ...
    if (this.outTexture && this.width === width && this.height === height && this.layers === layers && this.outFormat === format) {
      this.setFrame({ fullWidth: width, fullHeight: height, tileWidth: width, tileHeight: height, originX: 0, originY: 0, layerIndex: 0, originXf: 0.0, originYf: 0.0 });
      this._reprojFullW = width; this._reprojFullH = height;
      ...
      if (curFW !== (this._reprojFullW >>> 0) || curFH !== (this._reprojFullH >>> 0)) {
        this._dvReproj.setUint32(32, this._reprojFullW >>> 0, true); this._dvReproj.setUint32(36, this._reprojFullH >>> 0, true);
        this._writeIfChanged('reproj', this.reprojBuffer, this._abReproj); this._bg0Dirty = true;
      }
      return this.outView;
    }

    // destroy existing
    if (this.outTexture) try { this.outTexture.destroy(); } catch (_) { }
    this.outTexture = null; this.outView = null;
    ...
    this.width = width; this.height = height; this.layers = layers;
    ...
    // invalidate caches
    this._bg0Dirty = true; this._bg1Dirty = true; this._renderBgCache = new WeakMap(); this._renderBundleCache = new WeakMap(); this._lastHadWork = false;
    return this.outView;
  }

  setOutputView(view, { width, height, layers = 1, format = 'rgba16float' } = {}) {
    if (!view) throw new Error('setOutputView: view required');
    this.outTexture = null; this.outView = view; this.outFormat = format;
    if (width && height) {
      this.width = width; this.height = height; this.layers = layers;
      this.setFrame({ fullWidth: width, fullHeight: height, tileWidth: width, tileHeight: height, originX: 0, originY: 0, layerIndex: 0, originXf: 0.0, originYf: 0.0 });
      this._reprojFullW = width; this._reprojFullH = height;
    ...
    this._bg0Dirty = true; this._renderBgCache = new WeakMap(); this._renderBundleCache = new WeakMap(); this._lastHadWork = false;
    return this.outView;
  }
  ...
  _buildBg0Key() {
    const ids = [
      this._getResId(this.outView), this._getResId(this.optionsBuffer), this._getResId(this.paramsBuffer),
      this._getResId(this.dummyBuffer), this._getResId(this.offsetsBuffer), this._getResId(this.posBuffer),
      this._getResId(this.frameBuffer), this._getResId(this.historyOutView || this._dummyHistoryOutView),
      this._getResId(this.reprojBuffer), this._getResId(this.perfBuffer), this._getResId(this.tuningBuffer), this.outFormat
    ];
    return ids.join('|');
  }
  _buildBg1Key() {
    ...
    const ids = [
      this._getResId(this.weatherView), this._getResId(this._samp2D),
      this._getResId(this.shape3DView), this._getResId(this._sampShape),
      blueViewId, this._getResId(this._sampBN),
      this._getResId(this.detail3DView), this._getResId(this._sampDetail),
      this._getResId(this.lightBuffer), this._getResId(this.viewBuffer), this._getResId(this.boxBuffer),
      this._getResId(this.historyPrevView || this._dummyHistoryPrevView),
      this._getResId(this._samp2D), this._getResId(this.motionView || this._dummy2DMotionView),
      this._getResId(this._samp2D), this._getResId(this.depthPrevView || this._dummy2DDepthView),
  ...
  _ensureBlueView() {
    ...
    if (!this.blueTex) {
      ...
      this.blueTex = tex; this._ownsBlue = true;
    }
  ...
  _makeBindGroups() {
    ...
    if (!this._bg0Dirty && this._bg0Cache.has(k0)) this._currentBg0 = this._bg0Cache.get(k0);
      ...
      if (this._bg0Cache.has(k0) && this._bg0Dirty) { const idx = this._bg0Keys.indexOf(k0); if (idx >= 0) this._bg0Keys.splice(idx, 1); this._bg0Cache.delete(k0); }
      const bg0 = this._createBg0ForKey();
      this._bg0Cache.set(k0, bg0); this._bg0Keys.push(k0); this._currentBg0 = bg0; this._bg0Dirty = false;
      while (this._bg0Keys.length > 12) { const oldest = this._bg0Keys.shift(); this._bg0Cache.delete(oldest); }
    }
    ...
    if (!this._bg1Dirty && this._bg1Cache.has(k1)) this._currentBg1 = this._bg1Cache.get(k1);
      ...
      if (this._bg1Cache.has(k1) && this._bg1Dirty) { const idx = this._bg1Keys.indexOf(k1); if (idx >= 0) this._bg1Keys.splice(idx, 1); this._bg1Cache.delete(k1); }
      const bg1 = this._createBg1ForKey();
      this._bg1Cache.set(k1, bg1); this._bg1Keys.push(k1); this._currentBg1 = bg1; this._bg1Dirty = false;
      while (this._bg1Keys.length > 12) { const oldest = this._bg1Keys.shift(); this._bg1Cache.delete(oldest); }
    }
  }
  ...
  _projectWorldToPixel(p, view, width, height) {
    ...
    const tanYhalf = Math.tan(0.5 * (view.fovYRad || 0.5)); const aspect = view.aspect || 1;
    ...
    const px = (ndcX * 0.5 + 0.5) * width; const py = (ndcY * 0.5 + 0.5) * height;
  ...
  _computeAABBScreenRect(view, box, width, height, padPx = 4) {
    ...
    let minX = 1e9, minY = 1e9, maxX = -1e9, maxY = -1e9; let anyFront = false;
    for (const C of corners) {
      const q = this._projectWorldToPixel(C, view, width, height);
      if (!q.ok) continue; anyFront = true;
      if (q.x < minX) minX = q.x; if (q.y < minY) minY = q.y; if (q.x > maxX) maxX = q.x; if (q.y > maxY) maxY = q.y;
    }
    if (!anyFront) return { visible: false };
    minX = Math.max(0, Math.floor(minX) - padPx); minY = Math.max(0, Math.floor(minY) - padPx);
    maxX = Math.min(width, Math.ceil(maxX) + padPx); maxY = Math.min(height, Math.ceil(maxY) + padPx);
  ...
  async dispatchRect({ x, y, w, h, wait = false, coarseFactor = 1 } = {}) {
    ...
    // save full-res
    const savedFullW = this._reprojFullW || this.width; const savedFullH = this._reprojFullH || this.height;
    ...
    this.outTexture = this._coarseTexture; this.outView = this._coarseView; this.width = cW; this.height = cH; this.outFormat = (this._coarseTexture?.format || savedFormat);

    this.setFrame({ fullWidth: cW, fullHeight: cH, tileWidth: cW, tileHeight: cH, originX: baseX, originY: baseY, layerIndex: this._dvFrame.getInt32(36, true) | 0, originXf: 0.0, originYf: 0.0 });

    // ensure reproj UBO reflects full-res for normalization if necessary
    const curFW = this._dvReproj.getUint32(32, true) || 0, curFH = this._dvReproj.getUint32(36, true) || 0;
    if (curFW !== (savedFullW >>> 0) || curFH !== (savedFullH >>> 0)) {
      this._dvReproj.setUint32(32, savedFullW >>> 0, true); this._dvReproj.setUint32(36, savedFullH >>> 0, true);
      this._writeIfChanged('reproj', this.reprojBuffer, this._abReproj); this._bg0Dirty = true;
    }
    ...
    // restore outputs
    this.outTexture = savedOutTexture; this.outView = savedOutView; this.width = savedWidth; this.height = savedHeight; this.outFormat = savedFormat;

    // restore frame for full-res region and upsample
    this.setFrame({ fullWidth: savedWidth, fullHeight: savedHeight, tileWidth: w || savedWidth, tileHeight: h || savedHeight, originX: x || 0, originY: y || 0, layerIndex: this._dvFrame.getInt32(36, true) | 0, originXf: 0.0, originYf: 0.0 });
  ...
  async dispatchRectNoCoarse({ x, y, w, h, wait = false } = {}) {
    ...
    if (tw === 0 || th === 0) { this._lastHadWork = false; return this.outView; }

    this.setFrame({ fullWidth: this.width, fullHeight: this.height, tileWidth: tw, tileHeight: th, originX: baseX, originY: baseY, layerIndex: this._dvFrame.getInt32(36, true) | 0, originXf: 0.0, originYf: 0.0 });

    if (!this._reprojFullW) {
      this._dvReproj.setUint32(32, this.width >>> 0, true); this._dvReproj.setUint32(36, this.height >>> 0, true);
      this._writeIfChanged('reproj', this.reprojBuffer, this._abReproj); this._bg0Dirty = true;
    }
  ...
  async dispatchForBox({ padPx = 8, wait = false, coarseFactor = 1 } = {}) {
    ...
    const view = {
      ...
      fovYRad: this._dvView.getFloat32(64, true), aspect: this._dvView.getFloat32(68, true)
    };
    ...
    if (!rect.visible) { this._lastHadWork = false; return null; }
    const cf = Math.max(1, (coarseFactor | 0));
  ...
  async dispatch({ wait = false, coarseFactor = 1 } = {}) {
    const cf = Math.max(1, (coarseFactor | 0));
    if (cf >= 2) {
      const cW = Math.max(1, Math.ceil(this.width / cf)); const cH = Math.max(1, Math.ceil(this.height / cf));
      this._ensureCoarseTexture(cW, cH, this.layers);
      const savedFullW = this._reprojFullW || this.width, savedFullH = this._reprojFullH || this.height;
      const savedOutTexture = this.outTexture, savedOutView = this.outView, savedWidth = this.width, savedHeight = this.height, savedFormat = this.outFormat;
      this.outTexture = this._coarseTexture; this.outView = this._coarseView; this.width = cW; this.height = cH; this.outFormat = this._coarseTexture?.format || savedFormat;
      this.setFrame({ fullWidth: cW, fullHeight: cH, tileWidth: cW, tileHeight: cH, originX: 0, originY: 0, layerIndex: 0, originXf: 0.0, originYf: 0.0 });
      const curFW = this._dvReproj.getUint32(32, true) || 0, curFH = this._dvReproj.getUint32(36, true) || 0;
      if (curFW !== (savedFullW >>> 0) || curFH !== (savedFullH >>> 0)) { this._dvReproj.setUint32(32, savedFullW >>> 0, true); this._dvReproj.setUint32(36, savedFullH >>> 0, true); this._writeIfChanged('reproj', this.reprojBuffer, this._abReproj); this._bg0Dirty = true; }
      await this._dispatchComputeInternal({ wait });
      this.outTexture = savedOutTexture; this.outView = savedOutView; this.width = savedWidth; this.height = savedHeight; this.outFormat = savedFormat;
      await this._upsampleCoarseToOut({ srcX: 0, srcY: 0, srcW: cW, srcH: cH, dstX: 0, dstY: 0, dstW: this.width, dstH: this.height, wait });
      this._lastHadWork = true; return this.outView;
    }
    await this._dispatchComputeInternal({ wait }); this._lastHadWork = true; return this.outView;
  }
  ...
  _ensureRenderPipeline(format = 'bgra8unorm') {
    ...
    this._render = { pipe, bgl, samp, format }; return this._render;
  }

  _getOrCreateRenderBindGroup(canvas, bgl, samp) {
    let map = this._renderBgCache.get(canvas);
    if (!map) { map = new Map(); this._renderBgCache.set(canvas, map); }
    ...
    map.set(key, bg); if (map.size > 8) { const firstKey = map.keys().next().value; map.delete(firstKey); }
    return bg;
  }

  _getOrCreateRenderBundle(canvas, pipe, bgl, samp) {
    let map = this._renderBundleCache.get(canvas);
    if (!map) { map = new Map(); this._renderBundleCache.set(canvas, map); }
    ...
    rbe.setPipeline(pipe); rbe.setBindGroup(0, bg); rbe.draw(6, 1, 0, 0);
    const bundle = rbe.finish();
    map.set(key, bundle); if (map.size > 8) { const firstKey = map.keys().next().value; map.delete(firstKey); }
    return bundle;
  }
  ...
  _writeRenderUniforms(opts = {}) {
    ...
    const skyColor = opts.skyColor ?? [0.55, 0.70, 0.95];
    ...
    const wv3 = (ofs, v) => { dv.setFloat32(ofs, v[0], true); dv.setFloat32(ofs + 4, v[1], true); dv.setFloat32(ofs + 8, v[2], true); dv.setFloat32(ofs + 12, 0.0, true); };

    let camPos, right, up, fwd, fovYRad, aspect, sunDir;
    if (opts.cam && opts.cam.camPos && opts.cam.right && opts.cam.up && opts.cam.fwd) {
      camPos = opts.cam.camPos; right = opts.cam.right; up = opts.cam.up; fwd = opts.cam.fwd;
      fovYRad = (opts.cam.fovYDeg ?? 60) * Math.PI / 180; aspect = opts.cam.aspect ?? 1.0; sunDir = opts.sunDir ?? [0, 1, 0];
    } else {
      ...
      right = norm(cross(upRef, fwd)); up = cross(fwd, right);
      const zoom = opts.zoom ?? 3.0; camPos = [-fwd[0] * zoom, -fwd[1] * zoom, -fwd[2] * zoom];
      fovYRad = rad(opts.fovYDeg ?? 60); aspect = opts.aspect ?? 1.0;
      const sAz = rad(opts.sunAzimuthDeg ?? 45), sEl = rad(opts.sunElevationDeg ?? 20); const cel = Math.cos(sEl);
      sunDir = norm([cel * Math.sin(sAz), Math.sin(sEl), cel * Math.cos(sAz)]);
    }

    dv.setUint32(0, layerIndex, true); dv.setUint32(4, 0, true); dv.setUint32(8, 0, true); dv.setUint32(12, 0, true);
    wv3(16, camPos); wv3(32, right); wv3(48, up); wv3(64, fwd);
    dv.setFloat32(80, fovYRad, true); dv.setFloat32(84, aspect, true); dv.setFloat32(88, exposure, true); dv.setFloat32(92, sunBloom, true);
    wv3(96, opts.sunDir ?? [0, 1, 0]); wv3(112, skyColor);
  ...
  _ensureCanvasConfigured(canvas, format = 'bgra8unorm') {
    ...
    if (!ctxRec) { const ctx = canvas.getContext('webgpu'); if (!ctx) throw new Error('failed to get webgpu context'); ctxRec = { ctx, format: null }; this._ctxCache.set(canvas, ctxRec); }
    const { ctx } = ctxRec;
    let state = this._canvasStates.get(canvas);
    if (!state) { state = { lastSize: [0, 0], hasContent: false }; this._canvasStates.set(canvas, state); }
    if (state.lastSize[0] !== displayW || state.lastSize[1] !== displayH || ctxRec.format !== format) {
      ...
      state.lastSize = [displayW, displayH]; state.hasContent = false; ctxRec.format = format;
    }
    return { ctx, state };
  }

  renderToCanvasWorld(canvas, { layerIndex = 0, cam, sunDir = [0, 1, 0], exposure = 1.2, skyColor = [0.55, 0.70, 0.95], sunBloom = 0.0 } = {}) {
    ...
    if (!this._lastHadWork || !this.outView) {
      ...
      const pass = enc.beginRenderPass({ colorAttachments: [{ view: tex.createView(), loadOp: 'clear', clearValue: { r: skyColor[0], g: skyColor[1], b: skyColor[2], a: 1 }, storeOp: 'store' }] });
      pass.end(); this.queue.submit([enc.finish()]); state.hasContent = true; return;
    }
    ...
    pass.executeBundles([bundle]); pass.end(); this.queue.submit([enc.finish()]); state.hasContent = true;
  }
  ...
  renderToCanvas(canvas, opts = {}) {
    ...
    if (opts.displayWidth || opts.displayHeight) {
      ...
      canvas.style.width = `${Math.max(1, Math.floor(w))}px`; canvas.style.height = `${Math.max(1, Math.floor(h))}px`; canvas.style.removeProperty('aspect-ratio');
    }
    const { ctx, state } = this._ensureCanvasConfigured(canvas, format);
    const skyColor = opts.skyColor ?? [0.55, 0.70, 0.95];
    if (!this._lastHadWork || !this.outView) {
      const enc = this.device.createCommandEncoder(); const tex = ctx.getCurrentTexture();
      const pass = enc.beginRenderPass({ colorAttachments: [{ view: tex.createView(), loadOp: 'clear', clearValue: { r: skyColor[0], g: skyColor[1], b: skyColor[2], a: 1 }, storeOp: 'store' }] });
      pass.end(); this.queue.submit([enc.finish()]); state.hasContent = true; return;
    }
  ...
  _ensureCoarseTexture(w, h, layers = 1) {
    ...
    try { if (this._coarseTexture?.destroy) this._coarseTexture.destroy(); } catch (_) { }
    this._coarseW = w; this._coarseH = h; this._coarseLayers = layers;
    ...
    this._bg0Dirty = true; this._bg1Dirty = true;
  }
  ...
  _ensureUpsamplePipeline(format = this.outFormat) {
    ...
    const samp = this.device.createSampler({ magFilter: 'linear', minFilter: 'linear', addressModeU: 'clamp-to-edge', addressModeV: 'clamp-to-edge' });
    this._upsample = { pipe, bgl, samp, format }; return this._upsample;
  }
  ...
  _getOrCreateUpsampleBindGroup(coarseView, layerIndex) {
    ...
    const bg = this.device.createBindGroup({
      layout: up.bgl, entries: [
        { binding: 0, resource: up.samp }, { binding: 1, resource: coarseView }, { binding: 2, resource: { buffer: this.renderParams, offset: 0, size: 128 } }
      ]
    });
    this._upsampleBgCache.set(key, bg); if (this._upsampleBgCache.size > 32) { const firstKey = this._upsampleBgCache.keys().next().value; this._upsampleBgCache.delete(firstKey); }
    return bg;
  }
  ...
  async _upsampleCoarseToOut({ srcX = 0, srcY = 0, srcW, srcH, dstX = 0, dstY = 0, dstW, dstH, wait = false } = {}) {
    ...
    const enc = this.device.createCommandEncoder();
    for (let layer = 0; layer < this.layers; ++layer) {
      this._dvRender.setUint32(0, layer >>> 0, true); this._writeIfChanged('render', this.renderParams, this._abRender);
      ...
      pass.setPipeline(up.pipe); pass.setBindGroup(0, bg); pass.draw(6, 1, 0, 0); pass.end();
    }
    this.queue.submit([enc.finish()]);
    if (wait && typeof this.queue.onSubmittedWorkDone === 'function') await this.queue.onSubmittedWorkDone();
  }
  ...
  destroy() {
    ...
    for (const k of toDestroy) try { if (this[k]?.destroy) this[k].destroy(); } catch (_) { } finally { this[k] = null; }
    try { if (this.outTexture?.destroy) this.outTexture.destroy(); } catch (_) { }
    try { if (this._coarseTexture?.destroy) this._coarseTexture.destroy(); } catch (_) { }
    this.outTexture = null; this.outView = null; this._coarseTexture = null; this._coarseView = null;

    if (this._ownsBlue && this.blueTex) try { this.blueTex.destroy(); } catch (_) { }
    this.blueTex = null; this.blueView = null; this._ownsBlue = false;

    try { if (this._dummy2DMotion?.destroy) this._dummy2DMotion.destroy(); } catch (_) { }
    try { if (this._dummy2DDepth?.destroy) this._dummy2DDepth.destroy(); } catch (_) { }
    try { if (this._dummyHistoryPrev?.destroy) this._dummyHistoryPrev.destroy(); } catch (_) { }
    try { if (this._dummyHistoryOut?.destroy) this._dummyHistoryOut.destroy(); } catch (_) { }

    this._bg0Cache.clear(); this._bg0Keys.length = 0;
    this._bg1Cache.clear(); this._bg1Keys.length = 0;
    this._render = null; this._currentBg0 = null; this._currentBg1 = null;
    this._canvasStates = new WeakMap(); this._ctxCache = new WeakMap(); this._renderBgCache = new WeakMap(); this._renderBundleCache = new WeakMap(); this._upsampleBgCache.clear(); this._lastSums.clear();
  }
}
