// cloudTest.worker.js (full rewrite, seed-aware, 3D kernels, persistent offsets)
...
function maybeApplySeedToPermTable(params) {
  ...
  try { nb.buildPermTable?.(s); log('[SEED] Applied seed to perm table:', s); } catch (e) { console.warn('buildPermTable(seed) failed', e); }
}
...
async function bakeWeather2D(weatherParams = {}, force = false, billowParams = {}) {
  ...
  const baseView = await nb.computeToTexture(
    WEATHER_W, WEATHER_H, weatherParams,
  ...
  if (billowEnabled) {
    ...
    const bp = {
      zoom: typeof billowParams.zoom === 'number' ? billowParams.zoom : (weatherParams.zoom || 1.0),
      freq: typeof billowParams.freq === 'number' ? billowParams.freq : (weatherParams.freq || 1.0),
      octaves: typeof billowParams.octaves === 'number' ? billowParams.octaves : (weatherParams.octaves || 4),
      lacunarity: typeof billowParams.lacunarity === 'number' ? billowParams.lacunarity : (weatherParams.lacunarity || 2.0),
      seedAngle: typeof billowParams.seedAngle === 'number' ? billowParams.seedAngle : (weatherParams.seedAngle || 0.0),
      gain: typeof billowParams.gain === 'number' ? billowParams.gain : (weatherParams.gain || 0.5),
      threshold: typeof billowParams.threshold === 'number' ? billowParams.threshold : (weatherParams.threshold || 0.0),
      scale: typeof billowParams.scale === 'number' ? billowParams.scale : 1.0,
      ...
      seed: billowParams.seed !== undefined ? billowParams.seed : (weatherParams.seed !== undefined ? weatherParams.seed : undefined)
    };
...
function ensureHistoryTextures(w, h, layers = 1) {
  ...
  try { historyTexA?.destroy?.(); } catch (_) {} try { historyTexB?.destroy?.(); } catch (_) {}
...
function applyTileTransforms(tt, opts = {}) {
  // opts: { allowPositions, allowScale, allowVel, additive }
  ...
  if (allowScale && tt.shapeScale !== undefined) {
    const v = Number(tt.shapeScale); if (!Number.isNaN(v)) shapeScale = v;
  }
  if (allowScale && tt.detailScale !== undefined) {
    const v = Number(tt.detailScale); if (!Number.isNaN(v)) detailScale = v;
  }

  if (allowVel) {
    const sVel = readVec3(tt.shapeVel); if (sVel) shapeScrollVel = sVel;
    const dVel = readVec3(tt.detailVel); if (dVel) detailScrollVel = dVel;
  }
...
function getOrCreateRenderBundle(pipe, bgl, samp, format) {
  ...
  if (renderBundleCache.size > 12) { const first = renderBundleCache.keys().next().value; renderBundleCache.delete(first); }
  return { bundle, bg };
}
...
function applyWorkerTuning() {
  ...
  try { log('[TUNING] applying patch ->', workerTuning, 'version', workerTuningVersion); } catch {}
  try {
...
async function runFrame({
  ...
  // do not reset offsets here. Only update positions if caller is explicit.
  if (tileTransforms) {
  ...
  // volume, params, camera and light
  ...
  const right = norm(cross(upRef, fwd)); const up = cross(fwd, right);
  ...
  cb.setViewFromCamera({
    camPos: [preview?.cam?.x || 0, preview?.cam?.y || 0, preview?.cam?.z || 0], right, up, fwd,
  ...
  if (typeof queue.onSubmittedWorkDone === 'function') await queue.onSubmittedWorkDone(); else await new Promise(r => setTimeout(r, 8));
...
function payloadHasTuning(obj) { return !!(obj && obj.tuning && typeof obj.tuning === 'object'); }
...
function startLoop() {
  ...
  (async () => {
    ...
    if (workerReproj && workerReproj.enabled) { workerReproj = normalizeReproj(workerReproj); if (!workerReproj.frameIndex) workerReproj.frameIndex = 0; }
    while (loopEnabled) {
      const t0 = performance.now();
      try {
        ...
        if (workerReproj && workerReproj.enabled) {
          ...
          try { cb?.setReprojSettings?.(workerReproj); if (workerPerf) cb?.setPerfParams?.(workerPerf); if (cb) cb._bg0Dirty = cb._bg1Dirty = true; } catch (e) { console.warn('startLoop reproj apply failed', e); }
        }
        ...
        const fpsInst = 1000 / frameTime; emaFps = (emaFps === null) ? fpsInst : (emaFps * 0.92 + fpsInst * 0.08);
...
self.onmessage = async (ev) => {
  ...
  try {
    if (type === 'init') {
      ...
      dbg.weather = canvases.dbg.weather; dbg.weatherG = canvases.dbg.weatherG; dbg.shapeR = canvases.dbg.shapeR; dbg.detailR = canvases.dbg.detailR; dbg.blue = canvases.dbg.blue;
      SHAPE_SIZE = constants.SHAPE_SIZE; DETAIL_SIZE = constants.DETAIL_SIZE; WEATHER_W = constants.WEATHER_W; WEATHER_H = constants.WEATHER_H; BN_W = constants.BN_W; BN_H = constants.BN_H;
      await ensureDevice(); configureMainContext(); respond(true, { ok: true }); return;
    }

    if (type === 'resize') {
      const { main, dbg: dbgSize } = payload;
      MAIN_W = Math.max(1, main.width | 0); MAIN_H = Math.max(1, main.height | 0); DBG_W = Math.max(1, dbgSize.width | 0); DBG_H = Math.max(1, dbgSize.height | 0);
    ...
    if (type === 'bakeShape') {
      ...
      respond(true, { baked: 'shape128', timings }); return;
    }
    ...
    if (type === 'bakeDetail') {
      ...
      respond(true, { baked: 'detail32', timings }); return;
    }
    ...
    if (type === 'bakeAll') {
      ...
      respond(true, { baked: 'all', timings: { weather, blue, shape, detail, totalMs: t1 - t0 } }); return;
    }
    ...
    if (type === 'setSlice') {
      ...
      renderDebugSlices();
      respond(true, { slice: currentSlice }); return;
    }

    if (type === 'setReproj') {
      workerReproj = normalizeReproj(payload.reproj || null);
      workerPerf = payload.perf || workerPerf;
      if (workerReproj && workerReproj.enabled && (workerReproj.frameIndex === 0 || typeof workerReproj.frameIndex === 'undefined')) { workerReproj.frameIndex = 0; workerReproj.sampleOffset = 0; }
      if (cb) { if (workerPerf) cb.setPerfParams(workerPerf); if (workerReproj) { cb.setReprojSettings(workerReproj); cb._bg0Dirty = true; } }
      if (workerReproj && workerReproj.enabled) startLoop(); else stopLoop();
      respond(true, { ok: true, reproj: workerReproj, perf: workerPerf }); return;
    }
    ...
    if (type === 'setTuning') {
      ...
      respond(true, { ok: true, tuning: workerTuning, version: workerTuningVersion }); return;
    }
    ...
    if (type === 'setShapeScroll' || type === 'setDetailScroll' || type === 'setScroll') {
      ...
      if (type === 'setShapeScroll' && payload) {
        ...
        respond(true, { pos: shapeScrollPos, vel: shapeScrollVel }); return;
      }
      ...
      if (type === 'setDetailScroll' && payload) {
        ...
        respond(true, { pos: detailScrollPos, vel: detailScrollVel }); return;
      }
